/**
 * 파일시스템을 훑거나 움직이는 작업(스캔·실행·실행취소·휴지통 보내기)은 **한 번에 하나만** 돈다.
 *
 ...
 *
 * 네 작업이 서로의 플래그를 들여다보게 하면 검사가 열두 곳(4×3)이 되고 하나만 빠져도 조용히 깨진다.
 * 대신 자물쇠 하나를 같이 잡는다. 계획 세우기처럼 스스로는 잠그지 않되 아무 작업도 돌고 있지 않아야
 * 하는 곳은 assertIdle 로 본다.
 */
export type Activity = 'scan' | 'execute' | 'undo' | 'trash'

/** 오류 문장용. 조사까지 붙여 둔다 */
const SUBJECT: Record<Activity, string> = {
  scan: '스캔이',
  execute: '실행이',
  undo: '실행취소가',
  trash: '휴지통 보내기가'
}
const VERB: Record<Activity, string> = {
  scan: '스캔하세요',
  execute: '실행하세요',
  undo: '되돌리세요',
  trash: '휴지통으로 보내세요'
