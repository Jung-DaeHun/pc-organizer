...
export type TrashErrorCode =
  ...
  | 'exceeds-recycle-bin'
  /** 휴지통에 든 것과 보낼 것의 합계가 최대 크기 이상 — 탐색기가 나중에 오래된 것부터 영구 삭제하므로 보내지 않는다 */
  | 'recycle-bin-full'
  /** 이 드라이브가 '휴지통을 쓰지 않음'으로 설정돼 있다 — 삭제가 곧 영구 삭제 */
  | 'recycle-bin-off'
  /**
   * 휴지통 설정을 읽지 못했다 (드라이브 문자가 없는 경로, 조회 실패, 드라이브 아래 폴더에 마운트된 다른
   * 볼륨의 파일) — 영구 삭제일 수 있어 보내지 않는다
   */
  | 'recycle-bin-unknown'
  | 'io'
...
export const TRASH_ERROR_LABELS: Record<TrashErrorCode, string> = {
  ...
  'recycle-bin-full':
    '이 드라이브의 휴지통에 이미 든 것과 합치면 최대 크기 이상입니다 (윈도우가 오래된 항목부터 영구 삭제하므로 보내지 않습니다 — 휴지통을 비우거나 한 번에 보내는 그룹을 줄이세요)',
  'recycle-bin-off':
