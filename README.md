...
**지우는 기능은 없다.** 보고, 계획을 세우고, AI에게 물어본 뒤, 사용자가 확인한 이동만 실행하고
되돌린다.
...
파일을 지우는 기능은 아직 없다(되돌리기가 치우는 것은 자기가 만든 **빈** 폴더뿐이다). 중복 후보를
휴지통으로 보내는 기능은 전체 해시로 다시 확인하는 절차와 함께 따로 붙인다. 되돌릴 수 없는 동작을
...
- 중복 후보 정리 — 전체 해시로 다시 확인한 뒤 휴지통으로. AI 추천과는 무관한 규칙 기능. 그룹마다
  최소 하나는 남긴다
