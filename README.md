...
**아직 파일을 단 하나도 건드리지 않는다.** 보고, 계획을 세우고, AI에게 물어보는 것까지다.
...
- **정리 계획** — 감시 폴더 바로 아래의 파일·폴더를 어디로 옮길지 표로 보여준다. 확장자 규칙이
  초안을 만들고, 항목마다 체크박스와 목적지를 직접 고칠 수 있다
...
파일을 옮기고 지우는 기능은 계획 화면이 정확하다는 확신이 선 뒤에 붙인다. 되돌릴 수 없는 동작을
...
파일을 실제로 옮기는 기능. 안전장치가 먼저다.

- **승인 → 실행 → 실행취소** — 계획 화면에서 체크한 항목만, main이 가진 계획 원본과 대조한 뒤 옮긴다.
  이름을 바꾸지 않고, 덮어쓰지 않고, 다른 드라이브로 복사하지 않는다. 실행 기록을 남겨 되돌린다
- 중복 후보 정리 — 전체 해시로 다시 확인한 뒤 휴지통으로. AI 추천과는 무관한 규칙 기능
- 규칙 편집 UI, 설치된 앱 제거 안내, 시작 프로그램 켜고 끄기

`src/shared/types.ts`의 `ExecutionResult` / `UndoEntry`가 그 자리를 미리 잡아둔 타입이다.
